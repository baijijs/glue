...
const EXTRA_ENV_FILES = '*.env.{js,json}';
...
module.exports = function loadConfig(baseDir) {
  ...
  // Load [extra].env.{js,json}
  let extraEnvConfig = {};
  glob.sync(path.join(baseDir, EXTRA_ENV_FILES)).forEach(function(file) {
    extraEnvConfig = Object.assign(extraEnvConfig, require(file)[env.NODE_ENV]);
  });
